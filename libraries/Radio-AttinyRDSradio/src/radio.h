...
#define DEBUG_STR(txt)           if (_debugEnabled) {}

/// Used for Debugging function entries without parameters.
#define DEBUG_VAL(label, val)    if (_debugEnabled) {}
#define DEBUG_VALX(label, val)    if (_debugEnabled) {}

/// Used for Debugging function entries without parameters.
#define DEBUG_FUNC0(fn)          if (_debugEnabled) {}

/// Used for Debugging function entries with 1 parameter.
#define DEBUG_FUNC1(fn, p1)      if (_debugEnabled) {}

/// Used for Debugging function entries with 1 parameters as hex Value.
#define DEBUG_FUNC1X(fn, p1) if (_debugEnabled) {}

/// Used for Debugging function entries with 2 parameters.
#define DEBUG_FUNC2(fn, p1, p2)  if (_debugEnabled) {}

/// Used for Debugging function entries with 2 parameters and Hex Value.
#define DEBUG_FUNC2X(fn, p1, p2) if (_debugEnabled) {}
...
public:
  ...
  static void _wireWriteTo(int address, uint8_t *cmdData, int cmdLen);
  static uint8_t _wireReadFrom(int address, uint8_t *data, int len);

  // write a 16 bit value in High-Low order to the Wire.
  static void _write16HL(uint16_t val);
  static uint16_t _read16HL();
  ...
  */
   ...
   */
  int _wireRead(int address, uint8_t reg, uint8_t *data, int len);
   ...
   * @return number of register values received.
   */
  int _wireRead(int address, uint8_t *cmdData, int cmdLen, uint8_t *data, int len);
