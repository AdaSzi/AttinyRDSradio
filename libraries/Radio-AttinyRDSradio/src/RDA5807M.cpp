...
bool RDA5807M::init() {
  ...
  TinyWireM.begin();
  TinyWireM.beginTransmission(I2C_INDX);
...
RADIO_FREQ RDA5807M::getFrequency() {
  // check register A
  TinyWireM.requestFrom (I2C_SEQ, 2);
...
{
  TinyWireM.requestFrom (I2C_SEQ, (6 * 2) );
  for (int i = 0; i < 6; i++) {
...
void RDA5807M::_saveRegisters()
{
  ...
  TinyWireM.beginTransmission(I2C_SEQ);
  for (int i = 2; i <= 6; i++){  
...
{
  TinyWireM.write(val >> 8); TinyWireM.write(val & 0xFF);
} // _write16
...
void RDA5807M::checkRDS()
{
  ...
  if (_sendRDS) {
    ...
    if (registers[RADIO_REG_RA] & RADIO_REG_RA_RDS) {
      ...
      TinyWireM.beginTransmission(I2C_INDX);                // Device 0x11 for random access
      TinyWireM.write(RADIO_REG_RDSA);                   // Start at Register 0x0C
      TinyWireM.endTransmission(0);                         // restart condition
      
      TinyWireM.requestFrom(I2C_INDX, 8);                  // Retransmit device address with READ, followed by 8 bytes
      newData = _read16();
