...
bool RADIO::_wireExists(int address)
{
...
void RADIO::_wireWriteTo(int address, uint8_t *cmdData, int cmdLen)
{
  if (cmdData && cmdLen > 0) {
...
uint8_t RADIO::_wireReadFrom(int address, uint8_t *data, int len)
{
  uint8_t received = 0;
  if (data && len > 0) {
    while (!received) {
      received = TinyWireM.requestFrom(address, len);
      if (_wireDebugFlag) {
       //Serial.print('[');
...
void RADIO::_write16HL(uint16_t val)
{
  TinyWireM.write(val >> 8);
  TinyWireM.write(val & 0xFF);
} // _write16HL


// read a 16 bit value in High-Low order from the Wire.
uint16_t RADIO::_read16HL()
{
  uint8_t hiByte = TinyWireM.read();
  uint8_t loByte = TinyWireM.read();
 ...
 */
int RADIO::_wireRead(int address, uint8_t reg, uint8_t *data, int len)
{
  return (_wireRead(address, &reg, 1, data, len));
} // _wireRead()
 ...
 * @return number of register values received.
 */
int RADIO::_wireRead(int address, uint8_t *cmdData, int cmdLen, uint8_t *data, int len)
{
  int received = 0;

  RADIO::_wireWriteTo(address, cmdData, cmdLen);
  ...
  if (data) {
    while (received == 0) {
      ...
      received = RADIO::_wireReadFrom(address, data, len);
      if (!(*data & 0x80))
        received = 0;
    }
